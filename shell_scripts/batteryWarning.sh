...
then 
	if [ $battery_level -eq 20\
		-o $battery_level -eq 15\
		-o $battery_level -eq 10\
		-o $battery_level -eq 5 ]\
		&& [ ${batt:10} == "discharging" ]
	then
    	notify-send -u critical "Battery low" "Battery level is ${battery_level}%!"
    	play $audioFilePath

	else if [ $battery_level -eq 90\
		-o $battery_level -eq 95\
		-o $battery_level -eq 98\
		-o $battery_level -eq 99 ]\
		&& [ ${batt:10} == "charging" ]
	then
